...
class SettlementStatementControllerTest {
    ...
    void REPORT_REV_003_exposesSignedCompensatingEntry() throws Exception {
        ...
        mvc.perform(get("/api/v1/settlement-statements").param("from", "2030-01-15T00:00:00Z").param("to", "2030-01-17T00:00:00Z"))
                ...
                .andExpect(jsonPath("$.entries[0].entryId").value(entry.toString()))
                .andExpect(jsonPath("$.entries[0].entryType").value("REVERSAL"))
                .andExpect(jsonPath("$.entries[0].signedAmount").value("-975.61"))
                .andExpect(jsonPath("$.entries[0].settlementId").value(settlement.toString()));
    }
}
