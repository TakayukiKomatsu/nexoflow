...
class PricingControllerTest {
    ...
    void PRICE_001_returnsExactDecimalStringsForServerAuthoritativeSimulation() throws Exception {
        ...
        mvc.perform(post("/api/v1/pricing-simulations").contentType(MediaType.APPLICATION_JSON).content("{\"faceAmount\":\"1000.00\",\"faceCurrency\":\"BRL\",\"productType\":\"MERCANTILE_INVOICE\",\"dueDate\":\"2030-02-14\",\"settlementCurrency\":\"BRL\"}"))
                ...
                .andExpect(jsonPath("$.faceAmount").value("1000.0000"))
                .andExpect(jsonPath("$.termInMonths").value("1.0000000000"))
                .andExpect(jsonPath("$.fxRate").value("1"))
                .andExpect(jsonPath("$.settlementAmount").value("975.61"));
        verify(pricing).simulate(new PricingService.Input(
    ...
    @Test
    void PRICE_002_rejectsNumericFaceAmount() throws Exception {
        mvc.perform(post("/api/v1/pricing-simulations").contentType(MediaType.APPLICATION_JSON).content("{\"faceAmount\":1000.25,\"faceCurrency\":\"BRL\",\"productType\":\"MERCANTILE_INVOICE\",\"dueDate\":\"2030-02-14\",\"settlementCurrency\":\"BRL\"}"))
    ...
    @Test
    void PRICE_003_rejectsZeroFaceAmountString() throws Exception {
        mvc.perform(post("/api/v1/pricing-simulations").contentType(MediaType.APPLICATION_JSON).content("{\"faceAmount\":\"0\",\"faceCurrency\":\"BRL\",\"productType\":\"MERCANTILE_INVOICE\",\"dueDate\":\"2030-02-14\",\"settlementCurrency\":\"BRL\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("VALIDATION_FAILED"));
        verifyNoInteractions(pricing);
    }
    ...
    @Test
    void PRICE_004_rejectsFaceAmountStringWithMoreThanFourFractionDigits() throws Exception {
        mvc.perform(post("/api/v1/pricing-simulations").contentType(MediaType.APPLICATION_JSON).content("{\"faceAmount\":\"1.00001\",\"faceCurrency\":\"BRL\",\"productType\":\"MERCANTILE_INVOICE\",\"dueDate\":\"2030-02-14\",\"settlementCurrency\":\"BRL\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("VALIDATION_FAILED"));
        verifyNoInteractions(pricing);
    }
