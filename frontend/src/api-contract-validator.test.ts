...
describe("runtime OpenAPI contract validator", () => {
  ...
  it(
    "rejects primitive, optionality, array-reference, enum, and format drift",
    { timeout: 15_000 },
    async () => {
      const cases: Array<{
        expected: string;
  ...
  it(
    "rejects endpoint, request-body, query, header, and response drift",
    { timeout: 15_000 },
    async () => {
      const cases: Array<{
        expected: string;
        ...
        },
        {
          ...
          mutate: (document) => {
            ...
            if (parameters) {
              document.paths["/api/v1/settlement-statements"].get.parameters =
                parameters.filter(
                  (parameter) => parameter.name !== "assignorId",
                );
            }
          },
        },
