...
function extractObjectType(name) {
  ...
  if (!match) {
    fail(
      `could not find an object declaration for ${name} in frontend/src/api/client.ts`,
    );
  }
...
const missingQuoteFields = requiredQuoteFields.filter(
  (field) => !quoteBody.includes(field),
);
if (missingQuoteFields.length > 0) {
  fail(
    `PricingQuote is missing required top-level fields: ${missingQuoteFields.join(", ")}`,
  );
}
...
if (!pricingBreakdownDeclaration) {
  fail(
    "could not find the PricingBreakdown declaration in frontend/src/api/client.ts",
  );
}

const pricingBreakdown = pricingBreakdownDeclaration[1];
const omittedFields = [
  ...pricingBreakdown.matchAll(/"(productType|dueDate)"/g),
].map(([, field]) => field);
...
if (pricingBreakdown.startsWith("Omit<")) {
  ...
  if (missingOmissions.length > 0) {
    fail(
      `PricingBreakdown must omit inherited quote metadata fields: ${missingOmissions.join(", ")}`,
    );
  }
} else if (!pricingBreakdown.startsWith("{")) {
  fail(
    "PricingBreakdown must be an object type or explicitly omit inherited quote metadata",
  );
}
