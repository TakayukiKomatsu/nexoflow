...
| Hooks, CI, security, SBOM, licenses, docs | **Implemented** | Immutable CI actions, CodeQL job, Gitleaks/Trivy/Syft/license gates, schema/OpenAPI/link/claim validation; `.github/workflows/ci.yml`, `scripts/security-scan.sh` | `make security-scan && make validate-docs && make validate-traceability` |
| ADRs, schema, architecture flow, runbook, incident procedure, usage disclosure | **Implemented** | Full decision records, SQL/Java migration inventory, README runtime and settlement documentation, runbook, permission matrix, incident rollback procedure, and AI/tooling records; `README.md`, `docs/architecture/schema-inventory.md`, `docs/RUNBOOK.md`, `AI_USAGE.md`, `HT_USAGE.md` | `make validate-docs` |
| Evolution to 1M transactions/minute, EDA, partitioning | **Proposed** | Quantitative text design in `README.md`; explicitly not implemented and no production throughput/capacity proof | `make validate-docs` |
...
| --- | --- | --- | --- |
| DOC-LINK-001 | **Implemented** | `docs/sdd/11_sdd_staff-artifacts-and-reviewer-documentation.md`; `scripts/tests/test_architecture_docs.sh` | `make validate-docs`; README architecture and local-link checks |
| DOC-SCHEMA-002 | **Implemented** | `docs/sdd/11_sdd_staff-artifacts-and-reviewer-documentation.md`; `scripts/tests/test_architecture_docs.sh` | `make validate-docs`; migration-to-schema-inventory and OpenAPI checks |
